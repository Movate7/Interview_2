...
import { fetchCandidateBySerialNo, fetchCandidates } from "@/lib/googleSheets";
...
export default function QueuePage() {
  ...
  const { data: candidate, isLoading, error } = useQuery({
    queryKey: [`/api/candidates/serial/${serialNo}`],
    queryFn: () => fetchCandidateBySerialNo(serialNo || ""),
    enabled: !!serialNo
  });
  ...
  useEffect(() => {
    if (candidate && allCandidates.length > 0) {
      ...
      const position = sortedCandidates.findIndex(c => c.serialNo === candidate.serialNo);
      ...
      websocket.onmessage = (event) => {
        try {
          ...
          if (
            ...
            // Refresh data
            queryClient.invalidateQueries({ queryKey: [`/api/candidates/serial/${serialNo}`] });
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-neutral-600 mb-6">
                We couldn't find a candidate with serial number: {serialNo}
              </p>
              <Link 
                to="/"
  ...
  // Check if candidate has completed interviews (for feedback tab availability)
  const hasCompletedInterviews = candidate?.status === "completed" || candidate?.status === "rejected";

  // Determine initial active tab - show feedback if interviews are completed
  const [activeTab, setActiveTab] = useState<string>(hasCompletedInterviews ? "feedback" : "status");
      ...
      <div className="max-w-3xl mx-auto p-4">
        ...
        <Card className="mb-6 text-center">
          <CardContent className="pt-6">
            ...
            <h1 className="text-2xl font-medium text-neutral-800 mb-2">Walk-In Drive Portal</h1>
            <p className="text-neutral-600 mb-4">Track your interview status and provide feedback</p>
        ...
        <Tabs defaultValue={activeTab} className="mb-6" onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger 
              value="feedback" 
              disabled={!hasCompletedInterviews}
              title={!hasCompletedInterviews ? "Available after interview completion" : ""}
            >
              Provide Feedback
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="feedback" className="mt-4">
            {hasCompletedInterviews ? (
              <CandidateFeedbackForm candidate={candidate} />
            ) : (
              <Card>
                <CardContent className="pt-6 text-center">
                  ...
                  <h2 className="text-xl font-medium mb-2">Feedback Not Available Yet</h2>
                  <p className="text-neutral-600 mb-4">
                    You can provide feedback after completing all interview rounds.
                  </p>
                </CardContent>
              </Card>
            )}
